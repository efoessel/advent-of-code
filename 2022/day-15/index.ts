...
const lineForStep1 = process.argv[2] === 'example' ? 10 : 2000000;
    ...
    (sensors) => pipe(
        ...
        Arrays.flatMap((int, i, all) => {
            return all.slice(0, i).reduce((res, curr) => {
                return res.flatMap(i => i.exclude(curr));
            }, [int]);
        }),
        // now just count the size
        Arrays.map(i => i.length),
        Arrays.sum,
...
const algo2 = flow(
    ...
    (sensors) => pipe(sensors,
         // find pair of beacon that are distanced for each other so that the only let one space
        Arrays.getPairs,
...
assert(__dirname, algo1(lineForStep1), algo2, [5525847, 13340867187704]);
