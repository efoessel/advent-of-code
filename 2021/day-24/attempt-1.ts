 ...
 * Assumptions are only that we only input on w & that we reasonably can memo on the state (which is kinda true because of how it behaves)
 * 
 * About 7 minutes total on M3pro
 */
...
const findValidTail = (blocks: Instruction[], order: 'ASC' | 'DESC') => {
    ...
    // double memo otherwise we reach the max map size :grimacing:
    const findValidTail_rec = memo((tailLength: number) => memo(
        (state: ALUState): number[] | undefined => {
            ...
            for(const w of options) {
                ...
                const maybeValidTail = findValidTail_rec(tailLength-1)(nextState);
                if(maybeValidTail) {
    ...
    return findValidTail_rec(blocks.length-1)({w: 0, x: 0, y: 0, z: 0})!;
}
